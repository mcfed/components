...
const setup = (children, props) => {
  ...
  const wrapper = shallow(<FormItem>{children}</FormItem>, { context });
  return {
    props,
    wrapper
  };
...
const setupForm = (children, props) => {
  ...
  const wrapper = shallow(<BaseForm>{children}</BaseForm>, { context });
  return {
    props,
    wrapper
  };
...
describe("FormItem shallow render with Select", () => {
  ...
  it("FormItem render SELECT 带 fetch", done => {
    ...
    const { wrapper, props } = setup(
      <Select name="select" label="select" fetch={options} />
    );
  ...
  it("FormItem render SELECT 带 fetch url", done => {
    ...
    const { wrapper, props } = setup(
      <Select
        name="select"
        label="select"
        fetch={"http://192.168.200.178:3000/mock/63/capaa/v1/plugin/list"}
