...
describe('TreeView 是否渲染', () => {
  it('TreeView children TreeNode rendered', () => {
    const {wrapper, props} = setup({treeData, renderItem, checkedKeys: ['0-2']});
    expect(wrapper.find('ul').children().length).toBe(treeData.length);
  });

  it('TreeView render in modal', () => {
    const {wrapper, props} = setup({treeData, renderItem, checkedKeys: ['0-2'], isTreeInModal: true});
...
describe('TreeView events handle', () => {
  ...
  const {wrapper, props} = setup({treeData, renderItem, onSelect: jest.fn(), onChange: jest.fn()});
