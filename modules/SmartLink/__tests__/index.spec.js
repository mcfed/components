...
describe("smartlink unit case",()=>{
  ...
  it("smartlink render ", () => {
    ...
    expect(wrapper.instance().render()).toEqual(
      React.createElement(Link,Object.assign({},props),<div></div>)
    )
  ...
  it("smartlink resolveToLocation 方法 to是字符串时", () => {
    ...
    expect(wrapper.instance().resolveToLocation(params.to,params.router)).toEqual(
      [params.router.location.pathname,params.to].join("/")
    )
  });
  ...
  it("smartlink resolveToLocation to是json时", () => {
    ...
    expect(wrapper.instance().resolveToLocation(params.to,params.router)).toEqual(params.to)
  });
