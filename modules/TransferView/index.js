...
import { Transfer, Checkbox, Icon, Input } from 'antd'
import './style.css'
...
class NewTransferSearch extends Transfer.Search {
	...
	render = () => {
		...
		return React.createElement(
				'div',
				null,
				React.createElement(Input, { placeholder: placeholder, className: prefixCls, value: value, ref: ref => {this.refInput = ref}, onChange: this.handleChange }),
				icon
		);
	}
}
  ...
  render = () => {
    var _this3 = this;
		...
		var search = showSearch ? React.createElement(
				...
				React.createElement(NewTransferSearch, { prefixCls: prefixCls + '-search', onChange: this.handleFilter, handleClear: this.handleClear, placeholder: searchPlaceholder, value: filter }),
		) : null;
		...
		var checkAllCheckbox = React.createElement(Checkbox, { ref: ref => {this.refCheckbox = ref}, checked: checkedAll, indeterminate: checkStatus === 'part', onChange: function onChange() {
						return _this3.props.handleSelectAll(filteredDataSource, checkedAll);
				} });

		// header 重写
		var headerBody = this.props.header && this.props.header.map((value,i) => {
