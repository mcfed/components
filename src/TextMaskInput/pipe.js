export default function ipPipe() {
  return function(conformedValue) {
    const maxValue = 255;
    const indexesOfPipedChars = [];
    let conformedValueArr = conformedValue.split('.');

    conformedValueArr.forEach((val, index) => {
      const realVal = parseInt(val.replace('_', ''), 10);

      if (realVal > maxValue) {
        conformedValueArr[index] = maxValue;
        indexesOfPipedChars.push(index, index + 1, index + 2);
      }
    });
    console.log(indexesOfPipedChars);
    return {
