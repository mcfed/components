...
describe('BaseForm shallow render', () => {
  ...
  it('BaseForm render with props custom formLayout', done => {
    const instance = shallow(
      <BaseForm itemLayout={{labelCol: {span: 12}, wrapperCol: {span: 12}}}>
        <FormItem>
          <Input name='aaa' />
        </FormItem>
      </BaseForm>
    ...
    // instance.render()
    expect(instance.props('children').itemLayout).toHaveProperty('labelCol', {
      span: 12
    });
    expect(instance.props('children').itemLayout).toHaveProperty('wrapperCol', {
      span: 12
    });
    done();
  });
...
describe('FormItem render with formLayout ', () => {
  it('FormItem render labelCol wrapperCol', done => {
    ...
    expect(instance.find(FormItem).props()).toHaveProperty('labelCol', {
      span: 8
    });
    expect(instance.find(FormItem).props()).toHaveProperty('wrapperCol', {
      span: 16
    });
    done();
  });
